...
% pairwise-sorting
WTs_sorted= WT;
for cell_n=1:1:grid_OUTX*grid_OUTY-1
    
    img_candidate=  WT(cell_n, :);
    ...
    WTs_sorted(cell_n+1, :)= WTs_normed(sorted_w_indices(cell_n+1), :);
    WTs_normed(sorted_w_indices(cell_n+1), :)= NaN;
end;
...
clusters = kmeans(WTs_sorted, 12,'Replicates',10, 'Options',opts);
[vals, clusters_ind] = sort(clusters, 'ascend');
